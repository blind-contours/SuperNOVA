...
```{r basis function proportions}
indiv_shift_results %>%
  kbl(caption = "Effect Modification Results") %>%
...
```{r NIEH indiv results, fig.show="hold", eval = TRUE}
indiv_plots <- compute_meta_results(SuperNOVA_results = NIEH_results, parameter = "Indiv Shift")
names(indiv_plots$Plots)
indiv_plots$Plots$X7

indiv_plots$`Pooled Results`$X7 %>%
  kbl(caption = "Individual Stochastic Shift Results") %>%
  kable_classic(full_width = F, html_font = "Cambria")
```

```{r NIEH effect mod results, fig.show="hold", eval = TRUE}
effect_mod_plots <- compute_meta_results(SuperNOVA_results = sim_results, parameter = "Effect Mod")
names(effect_mod_plots$Plots)
effect_mod_plots$Plots$`Effect Mod`
```

```{r NIEH join results, fig.show="hold", eval = TRUE}
joint_plots <- compute_meta_results(SuperNOVA_results = sim_results, parameter = "Joint Shift")
names(joint_plots$Plots)
joint_plots$Plots$`Psi-X5&X7`
```
