context("Make sure CI folds coverage and pooled results cover ground-truth for two exposures for all parameters")
library(data.table)
set.seed(172943)
...
A_2 <- rpois(n, lambda = exp(4 + .1 * log(W$W1) - 0.2 * exp(W$W1) * W$W2)) ## moderate level of correlation 0.43
...
psi_tmle_results <- meta_results$`Pooled Results`$`Psi-A_1&A_2`
A1_tmle_results <- meta_results$`Pooled Results`$`A_1-A_1&A_2`
A2_tmle_results <- meta_results$`Pooled Results`$`A_2-A_1&A_2`
joint_tmle_results <- meta_results$`Pooled Results`$`A_1&A_2-A_1&A_2`


A1_shift_truth <- mean(shift_results$`A1 upshift` - shift_results$noshift)
A2_shift_truth <- mean(shift_results$`A2 upshift` - shift_results$noshift)
joint_shift_truth <- mean(shift_results$`A1 A2 upshift` - shift_results$noshift)
psi_truth <- joint_shift_truth - A1_shift_truth - A2_shift_truth
...
psi_pooled_CI_between <- between(psi_truth, psi_tmle_results$`Lower CI`[nrow(psi_tmle_results)], psi_tmle_results$`Upper CI`[nrow(psi_tmle_results)])

test_that("Pooled A1 CI bands cover the truth:", {
  expect_true(A1_pooled_CI_between)
})

test_that("Pooled A2 CI bands cover the truth:", {
  expect_true(A2_pooled_CI_between)
})

# test for reasonable equality between estimators
test_that("Pooled Joint CI bands cover the truth:", {
  expect_true(joint_pooled_CI_between)
})

# test for reasonable equality between estimators
test_that("Pooled interaction Psi CI bands cover the truth:", {
  expect_true(psi_pooled_CI_between)
})
