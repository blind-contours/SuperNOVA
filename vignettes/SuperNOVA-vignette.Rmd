...
This procedure is done in each fold of the CV procedure. Therefore, an estimate for each fold specific validation data is given. In order to optimize the optimum bias-variance trade-off for our causal parameter of interest we use targeted minimum loss based estimation (TMLE). 
...
# Lrnr_HAL_1 <- Lrnr_hal9001$new(max_degree = 1)
# Lrnr_HAL_2 <- Lrnr_hal9001$new(max_degree = 2, smoothness_orders = 0, num_knots = c(50, 25))
# Lrnr_HAL_3 <- Lrnr_hal9001$new(max_degree = 2, smoothness_orders = 0, num_knots = c(200, 100))
# Lrnr_HAL_4 <- Lrnr_hal9001$new(max_degree = 3, smoothness_orders = 0, num_knots = c(200, 100, 50))
# Lrnr_HAL_5 <- Lrnr_hal9001$new(max_degree = 3, smoothness_orders = 0, num_knots = c(300, 200, 100))
...
Lrnr_earth_4 <- Lrnr_earth$new(linpreds = FALSE, degree = 3, thresh = 0.0005)
Lrnr_earth_5 <- Lrnr_earth$new(linpreds = FALSE, degree = 4)
Lrnr_earth_6 <- Lrnr_earth$new(linpreds = FALSE, degree = 2,  fast.k	= 10)
Lrnr_earth_7 <- Lrnr_earth$new(linpreds = FALSE, degree = 2,  fast.k	= 5)
Lrnr_earth_8 <- Lrnr_earth$new(linpreds = FALSE, degree = 3,  fast.beta	= 0)
Lrnr_earth_9 <- Lrnr_earth$new(linpreds = FALSE, degree = 3, fast.k	= 10, pmethod="cv", nfold = 5)
Lrnr_earth_10 <- Lrnr_earth$new(linpreds = FALSE, degree = 2, pmethod = "exhaustive")
Lrnr_earth_11 <- Lrnr_earth$new(linpreds = FALSE, degree = 2, pmethod = "exhaustive", fast.k = 0)

two_ways <- combn(c("X1", "X2", "X3", "X4", "X5", "X6", "X7"), m = 2)
interactions <- lapply(seq_len(ncol(two_ways)), function(x) as.vector(two_ways[,x]))
...
lrn_lasso_interaction <- make_learner(Pipeline, lrn_interaction, lasso_learner)
lrn_ridge_interaction <- make_learner(Pipeline, lrn_interaction, ridge_learner)
lrn_enet_interaction <- make_learner(Pipeline, lrn_interaction, enet_learner)
lrn_ranger <- Lrnr_ranger$new(num.trees = 1000)
...
learners <- c(
  ...
  lrn_ridge_interaction,
  lrn_enet_interaction,
  lrn_ranger
)
...
## we will do the same thing to create a stack of MLE basis function models for the iterative backfitting of Y|W but we will limit the complexity of this SL a little because of usually higher dimensionality of W in real data compared to the previous stack for exposures: 

Lrnr_glm_basic <- Lrnr_glm$new()
Lrnr_mean_base <- Lrnr_mean$new()

# this is our estimator for the iterative backfitting on the exposures - the Super Learner made of basis function estimators, E(Y*|A,V)
...
# this is our estimator for outcome estimation E(Y|A,W) given shifts in the data-adaptively identified exposure variables
...
names(em_plots)
em_plots$`Effect Mod`
em_plots$`Level 1 Shift Diff in V`
em_plots$`Level 0 Shift Diff in V`
```
